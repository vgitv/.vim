...
function FortranGetFreeIndent()
    ...
    " -------------------------------------------------------------------------------------------------------
    if getline(v:lnum) =~? '^\s*\(#\|program\|module\|submodule\|'
    ...
    if getline(l:prev) =~? '^\s*\(program\|module\|contains\|'
                ...
                \ . 'type\s*\(::\|,\)\|enum\(,.*\|\s*::.*\)\|associate\)'
        " Indentation ++ début de bloc
        let l:ind += shiftwidth()
    endif

    " FERMETURE DU BLOC :
    " CONTAINS, ELSE, ELSE IF, END IF, END DO, END WHERE, CLOSE
    " END FUNCTION, END SUBROUTINE, END INTERFACE
    " END TYPE, END ENUM
    if getline(v:lnum) =~? '^\s*\(contains\|else\|else\s*if\|end\s*\(if\|do\|where\)\|close\|'
                \ . 'end\s*\(function\|subroutine\|interface\)\|'
                \ . 'end\s*\(type\|enum\|associate\)\)'
